...
-(void)setCircleRadius:(CGFloat)radius{
    if (radius > MAXDIS) {
        self.mapRadius = MAXDIS;
    } else if(radius < MINDIS) {
        self.mapRadius = MINDIS;
    } else {
        self.mapRadius = radius;
    }
    [self invalidatePath];
...
- (void)drawMapRect:(MKMapRect)mapRect
    ...
    CGFloat radiusAtLatitude = (self.mapRadius)*MKMapPointsPerMeterAtLatitude([[self overlay] coordinate].latitude);
    ...
    if (delegate && [delegate respondsToSelector:@selector(onRadiusChange:radius:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [delegate onRadiusChange:self radius:self.mapRadius];
        });
    }
